    ...
    def run(self):
        ...
        for page in range(1, self.pages+1):
            try:
                ...
                # 코스닥
                # url = f"https://finance.naver.com/sise/field_submit.naver?sosok=1menu=market_sum&returnUrl=http://finance.naver.com/sise/sise_market_sum.naver?sosok=1&page={page}&fieldIds=market_sum&fieldIds=debt_total&fieldIds=sales_increasing_rate&fieldIds=frgn_rate&fieldIds=per&fieldIds=roe"
                # 코스피100
                ...
                # trs = soup.select("div.box_type_m  > table.type_1 > tbody > tr[]")
                # response.raise_for_status()
                # print(trs)
                # exit()

                trs = soup.select("#contentarea > div.box_type_l >table.type_ > tbody > tr[onmouseover='mouseOver(this)']")
                response.raise_for_status()
