...
def chk_n_buy(stk_cd, token=None, log=None):
	message = ''	# 260602
	try:
	...
	if bid > 0:
		...
		if ord_qty == 0:
			message = "주문할 주식 수량이 0입니다." 
			print(message)
			log.error(message)
			return
		print('주문할 주식 수량:', ord_qty)
		log.info(f'주문할 주식 수량: {ord_qty}')
